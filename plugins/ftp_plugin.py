        ...
        try:
            ...
            if banner:
                findings.append(Finding(
                    target=target, port=port, protocol="ftp",
                    finding_type="banner", raw_value=banner,
                    context="FTP welcome banner", severity="info", plugin=self.name,
                ))
                for pattern_name, matched in find_patterns(banner):
                    findings.append(Finding(
                        target=target, port=port, protocol="ftp",
                        finding_type=pattern_name, raw_value=matched,
                        context="FTP banner", severity="high", plugin=self.name,
                    ))
